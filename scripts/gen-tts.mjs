...
async function synthGemini(text) {
  ...
  if (!part)
    throw new Error(
      "no audio in response: " + JSON.stringify(data).slice(0, 200),
    );
  const rate =
...
async function main() {
  ...
  const voiceLabel =
    PROVIDER === "gemini" ? `${GEMINI_MODEL}/${GEMINI_VOICE}` : CLOUD_VOICE;
  console.log(
  ...
  await writeFile(
    ...
    JSON.stringify(
      { provider: PROVIDER, voice: voiceLabel, ext: EXT, keys: present },
      null,
      0,
    ),
  );
  ...
  console.log(
    `Done. ${made} generated, ${skipped} already present, ${present.length} total.`,
  );

  if (DRY_RUN) {
    console.log("\nAudition these clips:");
    for (const { key } of SAMPLE_ITEMS)
      console.log(`  public/tts/${key}.${EXT}`);
    console.log(
...
if (isMain) {
  ...
  const keyVar =
    PROVIDER === "gemini" ? "GEMINI_API_KEY" : "GOOGLE_TTS_API_KEY";
  if (!key) {
    console.error(
